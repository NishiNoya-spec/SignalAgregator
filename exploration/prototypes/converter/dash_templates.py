...
dock_link = "http://tpqc-docs-zsmk-9684-dev.apps.ocpd." \
...
UPLOAD_INTERFACE = dcc.Upload(
    id="input-data",
    children=html.Div(["Перетяните файл или ",
...
CONVERTOR_PAGE_TEMPLATE = html.Div(
    [
        ...
        html.P(
            "Для конвертации данных подходят только .zip "
            "архивы полученные в результате работы нарезчика"
        ), UPLOAD_INTERFACE,
        dcc.Loading(
...
CONVERTOR_INTERFACE = [
    html.H4(
        "Выберите параметры конвертации, запустите вычисления, "
        "и ожидайте автоматической загрузки результата"
    ),
    html.Div(
        [
...
class InterfaceElementsGenerator:
    ...
    def set_data_paths_and_signal_names_for_aggregation(self, data_path):
        ...
        FORBIDDEN_NAMES = [
            "BilletPoint", "billet_points", "BilletPoints", "EndMoment"
        ]
        ...
        self.signal_names_for_aggregation = [
            signal_name for signal_name in signal_names_for_aggregation
            if signal_name not in FORBIDDEN_NAMES
        ]
